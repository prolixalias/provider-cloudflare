...
xpkg.release.publish.$(1).$(2):
	...
	@packages=""; \
	for p in $(XPKG_LINUX_PLATFORMS); do \
	  f="$(XPKG_OUTPUT_DIR)/$$$$p/$(2)-$(VERSION).xpkg"; \
	  [ -f "$$$$f" ] && packages="$$$$packages --package $$$$f"; \
	done; \
	if [ -z "$$$$packages" ]; then \
	  $(ERR) "no xpkg files found for $(2):$(VERSION); looked for: $(XPKG_LINUX_PLATFORMS)"; \
	  $(FAIL); \
	fi; \
	$(UP) xpkg push $$$$packages $(1)/$(2):$(VERSION) || $(FAIL)
